  ...
  <v-card id="house-card" class="pb-3">
    ...
    <div class="d-flex align-center">
      <v-card-title
        ...
        class="text-lg-h6 text-wrap"
        >{{ property?.name }}</v-card-title
      >
      <v-card-title v-else class="text-lg-h6 text-wrap"
        >{{ property?.name.slice(0, 10) }}...</v-card-title
      >
      <v-spacer></v-spacer>
      <div class="mr-3">
        <p
    ...
    <v-card-actions class="pa-0">
      <v-card-text class="text-green-accent-4 text-md-body-1"
        >${{ props.property?.price }}/month</v-card-text
      >
      <v-card-text class="text-end cursor-pointer" @click="isVisible()"
      ...
      >
        <v-card title="Rate this house !" max-width="400" class="">
          ...
          <v-card-actions class="px-5 mb-2 d-flex justify-end">
            <v-btn color="deep-orange-accent-3" @click="dialog = !dialog"
              >Cancel</v-btn
            >
            <v-btn
              color="green-accent-3"
              @click="rateStar(rating, props.property.id)"
              >Rate Now</v-btn
            >
          </v-card-actions>
        </v-card>
    ...
    <v-card-actions class="px-3">
      <v-icon
        color="green"
        size="default"
        class="mdi mdi-bookmark-outline mr-4"
